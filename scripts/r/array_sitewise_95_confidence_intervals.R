...
## Script purpose: Seeing the significance of various metrics across each of my networks WITHOUT INCLUDING YEAR
## Date: 14/06/18
...
r_network_gen <- function(input_network, collapse_species = T, desired_species = NULL, filter_species = F, include_malua=F, lulu= T){
  ...
  field_data$SiteAndYear <- field_data$Site
...
write.csv(real_and_errors, paste('data/output_data/randomized_ranges/sitewise_', chosen_ind, '.csv', sep = ''))
