#### Header ####
## Project: bat-diet
## Script purpose: visualising the family-level diets of each bat species for functional groups
## Date: 20/07/18
## Author: Dave Hemprich-Bennett (hemprich.bennett@gmail.com)
## Notes: the classification of the roles used is very crude and so this shouldn't be taken as anything too conclusive!
##################################################
...
tax_df <- t_taxa_mat[,c(seq(2,52),70, 86)]
tax_df <- melt(tax_df, id.vars = c('Species', "SiteAndYear"))
colnames(tax_df)[c(2,3,4)] <- c("SiteAndYear", 'Family', 'Present/absent')
...
prop_present <- sapply(unique(tax_df[,c('Species', 'Family', 'SiteAndYear')]),  function(x) as.character(x))
prop <- c()
nbats <- c()
#for(i in 1: 1){
for(i in 1: nrow(prop_present)){
  tem <- tax_df[which(tax_df$Species== as.character(prop_present[i,1])
                      ...
                      & tax_df$SiteAndYear==as.character(prop_present[i,3])),]
...
prop_present$role[prop_present$Family %in% c('Muscidae', 'Chloropidae', 'Stratiomyidae', 'Lycaenidae', 'Ptilodactylidae')] <- 'Didn`t classify'

#Remove Malua
prop_present <- prop_present[-which(prop_present$SiteAndYear=='MALUA, 2016'),]
prop_present$SiteAndYear <- gsub('DVCA', 'DANUM', prop_present$SiteAndYear)

site_facet <- ggplot(data = prop_present[which(prop_present$prop!=0),], aes(x = Family, y = fct_rev(for_x))) + 
  ...
  theme(panel.grid.major = element_blank(), axis.text.x = element_text(angle = 90, vjust = 0.5),
        ...
        strip.text.y = element_text(colour = 'white', size = 6),
        panel.background = element_rect(fill = "darkgray",
                                        colour = "darkgray",
                                        size = 0.5, linetype = NULL),
        strip.background =element_rect(fill="black", size = 3))+
  facet_grid(SiteAndYear ~role, scales = 'free', space="free", switch = 'both')#+

site_facet

pdf('plots/familyplots/crude_functional_roles.pdf', width = 12)
site_facet
dev.off()
