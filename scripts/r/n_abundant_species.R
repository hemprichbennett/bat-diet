...
ind <- c('functional complementarity', 'weighted NODF', 'modularity')
...
for(net_name in names(nets)){
  ...
  for(n_bats_kept in c(3:nrow(relative_abundance))){
    ...
    Modularity <- slot(computeModules(web = new_net), 
                'likelihood')
...
out_df <- out_df %>% pivot_longer(cols = c('WNODF', 'fun_comp', 'Modularity'),
