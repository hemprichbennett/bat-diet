...
library(here)
r_network_gen <- function(input_network, collapse_species = T, desired_species = NULL, filter_species = F, include_malua=F){
...
inter_network <- c()
n_prey_nodes <- c()
for(a in 1:length(netlists)){
  samples <- lapply(netlists[[a]], rownames)
  clust <- strsplit(names(netlists)[a], split = '\\/')[[1]][4]
  ...
  for(i in 1: nrow(t(combn(length(samples),2)))){
    ...
    inter_network <- c(inter_network, length(unique(overlappingnames)))
    n_prey_nodes <- c(n_prey_nodes, nrow(rawnets[[a]]))
  
}

# rowslist <- list()
# for(i in 1:length(netlists)){
#   #print(rownames(netlists[[i]][[1]]))
#   rowslist[[i]] <- lapply(netlists[[i]], rownames)
# }
# names(rowslist) <- names(netlists)
# 
# 
# clust <- c()
# inter_network <- c()
...
#   clust_level <- strsplit(names(rowslist)[i], split = '\\/')[[1]][4]
#   clust <- c(clust, clust_level)
#   inter_network <- c(inter_network, length(Reduce(intersect, rowslist[[i]])))
#   n_prey_nodes <- c(n_prey_nodes, nrow(rawnets[[i]]))
#   
# }
...
for(i in 1:length(netlists)){
  n_shared <- 0
  for(a in 1:length(netlists[[i]])){
    n_shared <- n_shared+ length(which(rowSums(netlists[[i]][[a]])>1))
  }
  intra_network <- c(intra_network, n_shared)
}

overlaps <- data.frame(clusts, inter_network, intra_network, n_prey_nodes)

write.csv(overlaps, 'data/output_data/raw_overlap.csv')
